...
@section('content')
    <div class="container mt-5">
        <h1 class="mb-4">Editar Evento: {{ $evento->titulo }}</h1>
        ...
        <form id="edit-event-form">
            ...
            <div class="mb-3">
                ...
                <input type="date" name="fecha" id="fecha" class="form-control" value="{{ old('fecha', $evento->fecha) }}" required min="{{ now()->toDateString() }}">
            </div>

            <!-- Hora de inicio -->
            <div class="mb-3">
                <label for="hora_inicio" class="form-label">Hora de inicio</label>
                <input type="time" name="hora_inicio" id="hora_inicio" class="form-control" value="{{ old('hora_inicio', $evento->hora_inicio) }}" required>
            </div>

            <!-- Hora de fin (Se calcula automáticamente) -->
            <div class="mb-3">
                <label for="hora_fin" class="form-label">Hora de fin</label>
                <input type="time" name="hora_fin" id="hora_fin" class="form-control" value="{{ old('hora_fin', $evento->hora_fin) }}" readonly required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="ponente_id" id="ponente_id" class="form-control" required>
                    ...
                    @foreach ($ponentes as $ponente)
                        <option value="{{ $ponente->id }}" {{ old('ponente_id', $evento->ponente_id) == $ponente->id ? 'selected' : '' }}>
                            {{ $ponente->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="capacidad_maxima" id="capacidad_maxima" class="form-control" value="{{ old('capacidad_maxima', $evento->capacidad_maxima) }}" required min="1">
            </div>
    ...
    <script>
        document.getElementById('hora_inicio').addEventListener('input', function() {
            ...
            if (horaInicio) {
                ...
                fecha.setMinutes(fecha.getMinutes() + 55); // +55 minutos
